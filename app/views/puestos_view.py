"""Vista de gestión de puestos: listado con búsqueda, paginación, creación y edición inline."""
...
from app.components.puestos_edit_modal import PuestosEditModal
...
class PuestosView(View):
    """Lista puestos en tabla paginada; permite buscar, crear, editar y eliminar."""

    key = "puestos"
    title = "Puestos"
    subtitle = "Gestión de puestos y funciones de la organización"
    ...
    def __init__(
        self,
        page: ft.Page,
        controller: Optional[PuestosController] = None,
        audit: Optional[AuditService] = None,
    ):
        super().__init__(page)
        self._controller = controller or PuestosController()
        self._audit = audit or AuditService()

        self._current_modal: Optional[PuestosEditModal] = None
        ...
        self._toolbar = TableToolbar(
            ...
            search_placeholder="Buscar por ID o nombre de puesto…",
            actions=[
    ...
    def _show_edit_modal(self, puesto: PuestosResponseDTO) -> None:
        """
        Abre el modal de edición para el puesto indicado.

        Argumentos:
            puesto (PuestosResponseDTO): Puesto cuyos datos se cargarán en el formulario.
        """
        try:
            self._current_modal = PuestosEditModal(
                puesto=puesto,
                on_save=lambda values: self._on_modal_save(puesto, values),
    ...
    def _show_create_modal(self) -> None:
        """Abre el modal para crear un nuevo puesto."""
        try:
            self._current_modal = PuestosEditModal(
    ...
    def _show_delete_confirm(self, puesto: PuestosResponseDTO) -> None:
        """
        Muestra un diálogo de confirmación antes de eliminar un puesto.

        Argumentos:
            puesto (PuestosResponseDTO): Puesto que se desea eliminar.
        """
        ...
        def confirm(_: ft.ControlEvent) -> None:
            close_dialog()
            self._delete_puesto(puesto)

        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Text("Eliminar puesto"),
            content=ft.Text(
                f"¿Seguro que deseas eliminar el puesto '{puesto.puesto}'?\n"
    ...
    def _on_modal_save(self, puesto: PuestosResponseDTO, form_values: dict[str, str]) -> None:
        """
        ...
        """
        ok, message = self._controller.save_puesto(puesto, form_values)
        self._close_modal()
        if ok:
    ...
    def _on_modal_create(self, form_values: dict[str, str]) -> None:
        ...
        Argumentos:
            form_values (dict[str, str]): Valores crudos capturados del formulario.
        """
        ok, message = self._controller.crear_puesto(form_values)
        self._close_modal()
        if ok:
            self._show_snackbar(f"✓ {message}")
            self._load_data()
    ...
    def _load_data(self) -> None:
        """Lanza la carga asíncrona de puestos desde el controller y actualiza la tabla al terminar."""
    ...
    def _delete_puesto(self, puesto: PuestosResponseDTO) -> None:
        """
        Elimina un puesto de forma asíncrona y recarga la tabla al finalizar.
        ...
        async def delete_async() -> None:
            try:
                ok, message = await asyncio.to_thread(self._controller.eliminar_puesto, puesto)
                if ok:
                    self._show_snackbar(f"✓ {message}")
                    self._load_data()
                else:
    ...
    def _build_row(self, item: PuestosResponseDTO) -> ft.Control:
        """
        Construye una fila de la tabla con los datos y botones de acción del puesto.

        Argumentos:
            item (PuestosResponseDTO): Puesto cuyos datos se representan en la fila.
    ...
    def _row_values(self, item: PuestosResponseDTO) -> list[str]:
        """
        Extrae los valores de celda de un puesto siguiendo el orden definido en _COLUMNS.

        Argumentos:
            item (PuestosResponseDTO): Puesto del que se extraen los valores.
