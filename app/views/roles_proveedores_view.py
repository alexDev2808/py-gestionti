"""Vista de gestión de roles de proveedores: listado con búsqueda, paginación, creación y edición."""
...
from app.components.roles_proveedores_edit_modal import RolesProveedoresEditModal
...
class RolesProveedoresView(View):
    """Lista roles de proveedores en tabla paginada; permite buscar, crear, editar y eliminar."""

    key = "roles_proveedores"
    title = "Roles Proveedor"
    subtitle = "Gestión de roles de proveedores"
    ...
    def __init__(
        self,
        page: ft.Page,
        controller: Optional[RolesProveedoresController] = None,
        audit: Optional[AuditService] = None,
    ):
        super().__init__(page)
        self._controller = controller or RolesProveedoresController()
        self._audit = audit or AuditService()
        self._current_modal: Optional[RolesProveedoresEditModal] = None
    ...
    def _show_edit_modal(self, item: RolesProveedoresResponseDTO) -> None:
        try:
            self._current_modal = RolesProveedoresEditModal(
                rol=item,
    ...
    def _show_delete_confirm(self, item: RolesProveedoresResponseDTO) -> None:
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(
                f"¿Seguro que deseas eliminar el rol '{item.rol}'?\n"
    ...
    def _on_modal_save(self, item: RolesProveedoresResponseDTO, form_values: dict[str, str]) -> None:
        ok, message = self._controller.save_rol(item, form_values)
        self._close_modal()
        if ok:
    ...
    def _on_modal_create(self, form_values: dict[str, str]) -> None:
        ok, message = self._controller.crear_rol(form_values)
        self._close_modal()
        if ok:
            self._show_snackbar(f"✓ {message}")
            self._load_data()
    ...
    def _delete_item(self, item: RolesProveedoresResponseDTO) -> None:
        async def delete_async() -> None:
            try:
                ok, message = await asyncio.to_thread(self._controller.eliminar_rol, item)
                if ok:
                    self._show_snackbar(f"✓ {message}")
                    self._load_data()
                else:
    ...
    def _build_row(self, item: RolesProveedoresResponseDTO) -> ft.Control:
    ...
    def _row_values(self, item: RolesProveedoresResponseDTO) -> list[str]:
        out: list[str] = []
