"""Vista de gestión de áreas: listado con búsqueda, paginación, creación y edición inline."""
...
from app.components.areas_edit_modal import AreasEditModal
...
class AreasView(View):
    """Lista áreas en tabla paginada; permite buscar, crear, editar y eliminar."""

    key = "areas"
    title = "Áreas"
    subtitle = "Gestión de áreas organizacionales"
    ...
    def __init__(
        self,
        page: ft.Page,
        controller: Optional[AreasController] = None,
        audit: Optional[AuditService] = None,
    ):
        super().__init__(page)
        self._controller = controller or AreasController()
        self._audit = audit or AuditService()

        self._current_modal: Optional[AreasEditModal] = None
    ...
    def _show_edit_modal(self, area: AreasResponseDTO) -> None:
        """
        Abre el modal de edición para el área indicada.

        Argumentos:
            area (AreasResponseDTO): Área cuyos datos se cargarán en el formulario.
        """
        try:
            self._current_modal = AreasEditModal(
                area=area,
                on_save=lambda values: self._on_modal_save(area, values),
    ...
    def _show_delete_confirm(self, area: AreasResponseDTO) -> None:
        """
        Muestra un diálogo de confirmación antes de eliminar un área.

        Argumentos:
            area (AreasResponseDTO): Área que se desea eliminar.
        """
        ...
        def confirm(_: ft.ControlEvent) -> None:
            close_dialog()
            self._delete_area(area)

        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Text("Eliminar área"),
            content=ft.Text(
                f"¿Seguro que deseas eliminar el área '{area.nombre}'?\n"
    ...
    def _on_modal_save(self, area: AreasResponseDTO, form_values: dict[str, str]) -> None:
        """
        ...
        """
        ok, message = self._controller.save_area(area, form_values)
        self._close_modal()
        if ok:
    ...
    def _on_modal_create(self, form_values: dict[str, str]) -> None:
        ...
        Argumentos:
            form_values (dict[str, str]): Valores crudos capturados del formulario.
        """
        ok, message = self._controller.crear_area(form_values)
        self._close_modal()
        if ok:
            self._show_snackbar(f"✓ {message}")
            self._load_data()
    ...
    def _load_data(self) -> None:
        """Lanza la carga asíncrona de áreas desde el controller y actualiza la tabla al terminar."""
    ...
    def _delete_area(self, area: AreasResponseDTO) -> None:
        """
        Elimina un área de forma asíncrona y recarga la tabla al finalizar.
        ...
        async def delete_async() -> None:
            try:
                ok, message = await asyncio.to_thread(self._controller.eliminar_area, area)
                if ok:
                    self._show_snackbar(f"✓ {message}")
                    self._load_data()
                else:
    ...
    def _build_row(self, item: AreasResponseDTO) -> ft.Control:
        """
        Construye una fila de la tabla con los datos y botones de acción del área.

        Argumentos:
            item (AreasResponseDTO): Área cuyos datos se representan en la fila.
    ...
    def _row_values(self, item: AreasResponseDTO) -> list[str]:
        """
        Extrae los valores de celda de un área siguiendo el orden definido en _COLUMNS.

        Argumentos:
            item (AreasResponseDTO): Área de la que se extraen los valores.
