"""Vista de gestión de proveedores: listado con búsqueda, paginación, creación y edición."""
...
from app.components.proveedores_edit_modal import ProveedoresEditModal
from app.components.table_toolbar import TableToolbar
from app.controllers.proveedores_controller import ProveedoresController
...
class ProveedoresView(View):
    """Lista proveedores en tabla paginada; permite buscar, crear, editar y eliminar."""
    ...
    def __init__(
        self,
        page: ft.Page,
        controller: Optional[ProveedoresController] = None,
        audit: Optional[AuditService] = None,
    ):
        super().__init__(page)
        self._controller = controller or ProveedoresController()
        self._audit = audit or AuditService()
        self._current_modal: Optional[ProveedoresEditModal] = None
        self._roles: list[RolesProveedoresResponseDTO] = []
        ...
        self._toolbar = TableToolbar(
            ...
            search_placeholder="Buscar por nombre, correo, origen o rol…",
            actions=[
    ...
    def _show_edit_modal(self, item: ProveedoresResponseDTO) -> None:
        if not self._roles:
            self._show_snackbar("Cargando roles, espera un momento…", error=False)
            return
        try:
            self._current_modal = ProveedoresEditModal(
                proveedor=item,
                roles=self._roles,
                on_save=lambda values: self._on_modal_save(item, values),
    ...
    def _show_delete_confirm(self, item: ProveedoresResponseDTO) -> None:
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Text(
                f"¿Seguro que deseas eliminar el proveedor '{item.nomprov}'?\n"
    ...
    def _on_modal_save(self, item: ProveedoresResponseDTO, form_values: dict[str, str]) -> None:
        ok, message = self._controller.save_proveedor(item, form_values)
        self._close_modal()
        if ok:
    ...
    def _on_modal_create(self, form_values: dict[str, str]) -> None:
        ok, message = self._controller.crear_proveedor(form_values)
        self._close_modal()
        if ok:
            self._show_snackbar(f"✓ {message}")
            self._load_data()
    ...
    def _load_data(self) -> None:
        ...
        async def load_async() -> None:
            try:
                ...
                roles = await asyncio.to_thread(self._controller.fetch_roles)
    ...
    def _delete_item(self, item: ProveedoresResponseDTO) -> None:
        async def delete_async() -> None:
            try:
                ok, message = await asyncio.to_thread(self._controller.eliminar_proveedor, item)
                if ok:
                    self._show_snackbar(f"✓ {message}")
                    self._load_data()
                else:
    ...
    def _build_row(self, item: ProveedoresResponseDTO) -> ft.Control:
    ...
    def _row_values(self, item: ProveedoresResponseDTO) -> list[str]:
        out: list[str] = []
