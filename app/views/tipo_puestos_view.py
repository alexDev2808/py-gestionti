"""Vista de gestión de tipos de puesto: listado con búsqueda, paginación, creación y edición."""
...
from app.components.tipo_puestos_edit_modal import TipoPuestosEditModal
...
class TipoPuestosView(View):
    """Lista tipos de puesto en tabla paginada; permite buscar, crear, editar y eliminar."""

    key = "tipo_puestos"
    title = "Tipos de Puesto"
    subtitle = "Gestión de tipos de puesto del personal"
    ...
    def __init__(
        self,
        page: ft.Page,
        controller: Optional[TipoPuestosController] = None,
        audit: Optional[AuditService] = None,
    ):
        super().__init__(page)
        self._controller = controller or TipoPuestosController()
        self._audit = audit or AuditService()
        self._current_modal: Optional[TipoPuestosEditModal] = None
    ...
    def _show_edit_modal(self, tp: TipoPuestosResponseDTO) -> None:
        try:
            self._current_modal = TipoPuestosEditModal(
                tipo_puesto=tp,
                on_save=lambda values: self._on_modal_save(tp, values),
    ...
    def _show_delete_confirm(self, tp: TipoPuestosResponseDTO) -> None:
        ...
        def confirm(_: ft.ControlEvent) -> None:
            close_dialog()
            self._delete_tipo_puesto(tp)

        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Text("Eliminar tipo de puesto"),
            content=ft.Text(
                f"¿Seguro que deseas eliminar '{tp.descp}'?\n"
    ...
    def _on_modal_save(self, tp: TipoPuestosResponseDTO, form_values: dict[str, str]) -> None:
        ok, message = self._controller.save_tipo_puesto(tp, form_values)
        self._close_modal()
        if ok:
    ...
    def _on_modal_create(self, form_values: dict[str, str]) -> None:
        ok, message = self._controller.crear_tipo_puesto(form_values)
        self._close_modal()
        if ok:
            self._show_snackbar(f"✓ {message}")
            self._load_data()
    ...
    def _delete_tipo_puesto(self, tp: TipoPuestosResponseDTO) -> None:
        async def delete_async() -> None:
            try:
                ok, message = await asyncio.to_thread(self._controller.eliminar_tipo_puesto, tp)
                if ok:
                    self._show_snackbar(f"✓ {message}")
                    self._load_data()
                else:
    ...
    def _build_row(self, item: TipoPuestosResponseDTO) -> ft.Control:
    ...
    def _row_values(self, item: TipoPuestosResponseDTO) -> list[str]:
        out: list[str] = []
